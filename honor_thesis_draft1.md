...
for a given word, $w_o$. In this setting, $w_o$ represents a vector of zeros with length $V$, where the $o^{th}$ entry is $1$. 
...
We have seen the value we wish to maximize for a single example. However, in terms of the entirety of our corpus, we wish to find the $\theta$ which maximizes:
...
#### 1.1.2    Continuous Bag-of-Words Architecture
...
$$ \{(w_{o-j},\ w_o)\ |\ 0\leq o\leq W,\ -c\leq j \leq c,\ j\neq 0\} $$
...
$$y = \begin{bmatrix}
         ...
         \end{bmatrix} $$
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTExMDg0MzA3NTBdfQ==
-->