...
One may notice in the Skip-Gram model the input-output pairs are both of the same dimension. This is not true of the CBOW model. So, we define a function $g:\ \mathbb{R}^{2c-1\times f}$ to be a a concatenating or averaging function which then is multiplied  on a word-level the goal becomes to maximize:
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEwNDIyNDQ5NTgsMTIzNDA1Njc2MCwxNT
